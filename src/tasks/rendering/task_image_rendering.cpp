...
void fetchNetatmo();
void fetchOwm();
uint16_t elapsedMinutes(uint32_t from)
{
    auto elapsedMillis = millis() - from;
...
[[noreturn]] void imageRenderingTask(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (elapsedMinutes(lastNetatmoUpdate) >= NETATMO_UPDATE_INTERVAL_MINUTES || lastNetatmoUpdate == 0)
        {
            fetchNetatmo();
            needUpdate = true;
        }
